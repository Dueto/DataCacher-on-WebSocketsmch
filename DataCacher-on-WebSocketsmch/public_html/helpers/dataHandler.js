(function(window)
{
    var dataHandler = function()
    {
        ...
        me.NeedenInformation = new ArrayBuffer();
        me.DateTime = new ArrayBuffer();
        me.data = new ArrayBuffer();
...
//            {level: 'ThreeHour', aggregator: ':00:00.000000', window: 25}];
        ...
        me.startBackgroundCaching = function(level, tableColumns, tableName)
        {
            ...
            {
                this.backCachers.push(new Worker('./datacaching/backgrDataCacher.js'));
                this.backCachers[this.backCachers.length - 1].postMessage(this.db_server + '<>' + this.db_name + '<>' + this.db_group + '<>' + this.window + '<>' + level.window + '<>' + tableColumns + '<>' + tableName);
            }
        };
